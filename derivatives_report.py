...
def trigger(myID, myYYY, myMM):
    ...
    elif socket.gethostbyname(socket.gethostname())=='172.27.110.106': 	#Jay's pc
        ipath='D:\\Python\\Python35\\Scripts\\phantomjs.exe'
    ...
    Select(driver.find_element_by_id('month')).select_by_visible_text(myMM)
...
def iteration(arr, start_from=0, end_to=9999):
    myYYY=str(int(time.strftime("%Y"))-1911)
    myMM=str(int(time.strftime("%m"))-1)
    myID=''
